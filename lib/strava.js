...
// Fetch the activity's data streams (altitude, latlng, distance). Used to
// compute real per-trail elevation profiles by slicing on segment_effort
// start_index / end_index. Returns null on failure (we degrade gracefully).
export async function fetchActivityStreams(accessToken, activityId) {
  try {
    const url = `${STRAVA_API_BASE}/activities/${activityId}/streams?keys=altitude,distance,latlng&key_by_type=true`;
