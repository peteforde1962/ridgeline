...
function countRideHits(ridePoints, trail, thresholdM = 35) {
...
export async function detectTrailsForActivity({ supabase, userId, activity, userTrails }) {
  ...
  // Tier 2: GPS match. Require >= 3 ride sample points within 35m of trail
  // geometry. This is tighter again — looser thresholds caught flyby trails.
  const gpsHits = [];
  if (ridePoints.length > 0) {
    for (const t of osmTrails) {
      const h = countRideHits(ridePoints, t);
      if (h >= 3) gpsHits.push({ trail: t, points: h });
    }
  }
