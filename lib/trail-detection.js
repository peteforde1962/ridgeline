...
//
// `osmCache` (optional) lets the caller share the OSM trails-near-area lookup
// across multiple activities so we don't re-query Overpass per ride. Pass an
// object with `.byKey` (Map) and the function will populate it.
export async function detectTrailsForActivity({ supabase, userId, activity, userTrails, osmCache }) {
  ...
  // For Tier 0 segment-to-OSM cross-referencing: try to fetch OSM trails for
  // the ride area to map Strava community names to canonical Trailforks/OSM names.
  // Hard-capped to 4s per call; if Overpass is slow we skip cross-ref for this
  // ride (and the rest of the sync) rather than blowing the function timeout.
  // Uses caller-provided cache so we don't repeat the call across rides in the
  // same area.
  const OSM_TIMEOUT_MS = 4000;
  async function getOsmForArea() {
    const startLatLng = activity?.start_latlng;
    if (!Array.isArray(startLatLng) || startLatLng.length < 2) return [];
    // Bucket by ~5km grid so nearby rides share a cache hit.
    const key = `${startLatLng[0].toFixed(2)},${startLatLng[1].toFixed(2)}`;
    if (osmCache?.byKey?.has(key)) return osmCache.byKey.get(key);
    // If we've already had ANY OSM timeout in this sync, skip — Overpass is
    // having a bad day and we don't want to compound it.
    if (osmCache?.timedOut) return [];
    try {
      const result = await Promise.race([
        fetchOsmTrails({ lat: startLatLng[0], lon: startLatLng[1], radiusKm: 15 }),
        new Promise((_, rej) => setTimeout(() => rej(new Error("OSM Overpass timed out")), OSM_TIMEOUT_MS)),
      ]);
      if (osmCache?.byKey) osmCache.byKey.set(key, result);
      return result;
    } catch (e) {
      if (osmCache) osmCache.timedOut = true;
      details.osm_crossref_skipped = e.message;
      return [];
    }
  }
  ...
  if (Array.isArray(activity?.segment_efforts) && activity.segment_efforts.length > 0) {
    ...
    // Multiple Strava segments can resolve to the SAME canonical trail (e.g.
    // "Pseudo Tsuga new berms" + "Pseudo Tsuga lower" both → "Pseudo Tsuga").
    // Aggregate into a map keyed by trail id so we don't violate the
    // ride_trails primary key on insert.
    const matchByTrailId = new Map();
    function recordMatch(trailId, seconds) {
      const existing = matchByTrailId.get(trailId);
      if (existing) {
        existing.points += 1;
        existing.seconds_on_trail = (existing.seconds_on_trail || 0) + (seconds || 0);
      } else {
        matchByTrailId.set(trailId, { trailId, points: 1, seconds_on_trail: seconds });
      }
    }
    ...
    for (const seg of byId.values()) {
      ...
      if (!trail) {
        insertAttempted++;
        const result = await insertTrailResilient(supabase, {
          user_id: userId,
          name: seg.name,
        ...
        if (!result.data) {
          insertFailed++;
          failures.push({ name: seg.name, error: result.error });
          continue;
        }
        trail = result.data;
      ...
      recordMatch(trail.id, seg.seconds);
    }

    const matches = [...matchByTrailId.values()];
    details.stravaSegments = byId.size;
    details.stravaSegmentsSkipped = skippedNonRide;
    details.osmCrossReferenced = crossRefHits;
    details.trailInsertAttempted = insertAttempted;
    details.trailInsertFailed = insertFailed;
    details.uniqueTrailIds = matches.length;
    if (failures.length > 0) details.trailInsertFailures = failures.slice(0, 5);
    if (matches.length > 0) {
      return {
        matches,
