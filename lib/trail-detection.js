...
export async function detectTrailsForActivity({ supabase, userId, activity, userTrails }) {
  ...
  if (Array.isArray(activity?.segment_efforts) && activity.segment_efforts.length > 0) {
    ...
    for (const seg of byId.values()) {
      // Skip non-cycling segments (Strava segments can be hiking/running too).
      if (seg.activity_type && seg.activity_type !== "Ride") continue;
  ...
  // then filter: keep top 12 by hit count AND require each to have at least
  // 15% of the leader's hit count. Cuts out the long tail of flyby trails.
  let gpsHits = [];
  ...
  const minRequired = Math.max(2, Math.floor(maxHits * 0.15));
  gpsHits = gpsHits.filter((h) => h.points >= minRequired).slice(0, 12);
