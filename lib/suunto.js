...
const SUUNTO_ACTIVITY_MAP = {
  ...
  // NOTE: Suunto's activityId codes vary by device firmware / API tier. Our
  // best-effort labels below are corrected based on real tester data.
  // Fallback for unknown IDs = "Activity N" with kind "other".
  10: { sport_type: "MountainBikeRide",  kind: "cycle",    label: "MTB" },
  11: { sport_type: "EMountainBikeRide", kind: "cycle",    label: "E-MTB" },
  ...
  // Suunto Vertical (newer firmware) uses 106 for MTB per tester data.
  106:{ sport_type: "MountainBikeRide",  kind: "cycle",    label: "MTB" },
};
...
// Fetch recent workouts. `since` is a unix-ms timestamp; pass null to skip
// the filter and get the most-recent N workouts (safer for first sync).
// Returns { workouts, requestUrl, rawShape } so callers can surface debug info.
export async function fetchWorkouts(accessToken, { since = null, limit = 100 } = {}) {
  const params = new URLSearchParams();
  if (since != null) params.set("since", String(since));
  params.set("limit", String(limit));
  const requestUrl = `${SUUNTO_API}/workouts?${params}`;
  const res = await fetch(requestUrl, { headers: apiHeaders(accessToken) });
  if (!res.ok) throw new Error("Suunto workouts fetch failed: " + await res.text());
  const data = await res.json();

  // Suunto response has historically wrapped workouts in `payload`, but we've
  // seen the shape vary across tiers — accept a few common variants so the
  // sync doesn't silently return empty when the API changes.
  const workouts =
  ...
  const rawShape = Array.isArray(data)
    ? "array"
    : (data && typeof data === "object" ? Object.keys(data).slice(0, 6).join(",") : typeof data);

  return { workouts, requestUrl, rawShape };
}

// Compute the workout's LOCAL date (not UTC). Suunto returns startTime as an
// epoch ms in UTC + a timezoneOffset in minutes. Without applying the offset,
// evening workouts in negative-offset timezones (e.g. Vancouver) land on the
// next UTC day and end up on the wrong calendar cell.
function localDateFromWorkout(workout) {
  const startMs = workout.startTime || 0;
  const offsetMinutes = Number(workout.timezoneOffset ?? workout.timeOffset ?? 0);
  const local = new Date(startMs + offsetMinutes * 60 * 1000);
  // We already shifted to local — pull UTC parts so no second TZ conversion happens.
...
// Suunto sends duration under a few different field names depending on the
// API tier / version — accept any of them and normalize to seconds.
function durationSeconds(workout) {
  const raw = workout?.totalTime
           ?? workout?.duration
  ...
  // Heuristic: if the number is huge, it's probably in milliseconds not
  // seconds (anything > 24h in seconds is suspicious for a workout).
  return n > 86400 ? Math.round(n / 1000) : Math.round(n);
}

// Why did we skip this workout? Returned instead of a row so the debug output
// tells us specifically what was missing rather than a generic "non-cycling".
export function workoutSkipReason(workout) {
  if (!workout) return "empty workout object";
  const secs = durationSeconds(workout);
  if (secs == null)     return `no duration field (checked totalTime/duration/totalDuration/movingTime)`;
  if (secs < 60)        return `duration under 1 min (${secs}s)`;
  return null;   // no reason to skip — will import
}

// Convert Suunto workout → rides table row. Accepts all activity types.
// Returns null when the workout is unusable (checked via workoutSkipReason).
export function workoutToRide(workout, userId) {
  if (workoutSkipReason(workout)) return null;
  const secs = durationSeconds(workout);
  const map = SUUNTO_ACTIVITY_MAP[workout.activityId] || {
    sport_type: null, kind: "other", label: `Activity ${workout.activityId ?? "?"}`,
  };
  // Prefer a user-set description over our (potentially-stale) activity map
  // label when we're not confident what the activityId maps to. If both are
  // present, we lead with the description and keep the map label as backup.
  const userTitle = (workout.description || workout.activityName || "").trim();
  const preferredLabel = userTitle || map.label;
  return {
    user_id: userId,
    suunto_workout_key: workout.workoutKey || String(workout.id),
    source: "suunto",
    date: localDateFromWorkout(workout),
    ...
    feel: null,
    // Prefer the user's own description over our map label when we have both.
    // Falls back to the mapped label. Result: activities always show a real
    // name even when our activity-code map is out of date.
    notes: preferredLabel || null,
