...
// Tight query — just MTB-specific tags. Broader queries time out the function.
function buildQuery(lat, lon, radiusKm) {
  const radius = Math.round(radiusKm * 1000);
  return `[out:json][timeout:25];(way["mtb:scale"](around:${radius},${lat},${lon});way["route"="mtb"](around:${radius},${lat},${lon}););out tags geom 200;`;
}
...
export function osmToTrailRow(osm, userId, regionLabel) {
  return {
    ...
    // Persist the polyline so we can sample real DEM elevation later.
    geometry:  osm.geometry || null,
  };
}
