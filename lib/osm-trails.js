...
// We keep this fairly tight to stay under Vercel's 10s function timeout on Hobby plan.
function buildQuery(lat, lon, radiusKm) {
  const radius = Math.round(radiusKm * 1000); // meters
  return `[out:json][timeout:25];(way["mtb:scale"](around:${radius},${lat},${lon});way["route"="mtb"](around:${radius},${lat},${lon}););out tags geom 150;`;
}
...
function normalizeTrails(elements) {
  ...
  for (const el of elements) {
    ...
    if (!name) continue; // skip nameless ways — too noisy

    const lengthM = el.geometry ? polylineLength(el.geometry) : 0;
  ...
  return Array.from(byName.values()).map((e) => ({
    ...
    elev_m: null,                                // OSM rarely tags elevation gain
