...
export function buildPlan(profile) {
  ...
  // Days the rider has elected to train. Unchecked days become rest days.
  // Defaults to all 7 for backwards compatibility (older profiles without the column).
  const workoutDays = Array.isArray(profile?.workout_days) && profile.workout_days.length > 0
    ? new Set(profile.workout_days)
    : new Set([0, 1, 2, 3, 4, 5, 6]);

  // Activity types the rider opted IN to. Anything not in this set gets
  // stripped from the template. Defaults to all types so existing plans keep
  // their shape unchanged.
  const enabledActivities = Array.isArray(profile?.enabled_activities) && profile.enabled_activities.length > 0
    ? new Set(profile.enabled_activities)
    : new Set(["ride", "strength", "yoga", "run", "rope"]);
    ...
    for (let j = 0; j < allocations[pi]; j++) {
      weeks.push({
        ...
        days: PLAN_TEMPLATE[phase.key].map((sessions, dayIdx) => {
          // If the rider didn't pick this day of the week, force rest.
          const isWorkoutDay = workoutDays.has(dayIdx);
          // Filter out any session type the rider opted out of ("no yoga",
          // "just ride + strength", etc.). If nothing survives, day is rest.
          const filtered = sessions.filter((s) => s === "rest" || enabledActivities.has(s));
          const finalSessions = filtered.length > 0 ? filtered : ["rest"];
          return {
            day: DAY_NAMES[dayIdx],
            sessions: isWorkoutDay ? finalSessions : ["rest"],
            details: isWorkoutDay ? workoutDetailsFor(phase.key, dayIdx, finalSessions) : [],
          };
        }),
      });
    }
...
// Is a specific date within any of the user's plan_pauses ranges?
// `pauses` is an array of { starts_on, ends_on } (YYYY-MM-DD strings).
// dateStr is a YYYY-MM-DD string. Comparison is inclusive on both ends.
export function isOnHoliday(dateStr, pauses) {
  if (!dateStr || !Array.isArray(pauses) || pauses.length === 0) return false;
  return pauses.some((p) =>
    p?.starts_on && p?.ends_on &&
    dateStr >= p.starts_on && dateStr <= p.ends_on
  );
}

// Returns the currently-active pause (if any) so the UI can show its reason.
export function activePause(dateStr, pauses) {
  if (!dateStr || !Array.isArray(pauses)) return null;
...
// Where is the user in the plan lifecycle right now?
//   "none"     — no started_at set (never started or was reset)
//   "active"   — inside the plan window (today falls within week 0..plan.length-1)
//   "complete" — today is past the end of the plan
// Used by /today, /plan, and the dashboard so we can render the right empty
// states instead of pretending an expired plan is still active.
export function planStatus(profile, plan) {
  if (!profile?.started_at) return "none";
  ...
  const diffDays = Math.floor((today - start) / 86400_000);
  if (diffDays < 0) return "none";              // profile.started_at in the future
  const currentWeek = Math.floor(diffDays / 7);
  return currentWeek >= plan.length ? "complete" : "active";
}
...
// Which day of the week (0=Mon..6=Sun) for a given IANA timezone?
// Falls back to the server's local time if no timezone provided.
export function todayDayIndex(timezone) {
  const now = new Date();
  let dayShort;
  try {
    ...
    dayShort = new Intl.DateTimeFormat("en-US", { weekday: "short" }).format(now);
  }
...
export function todayDateInTz(timezone) {
  const now = new Date();
  try {
