...
import { buildPlan, currentWeekIndex, todayDayIndex, todayDateInTz, sessionLabel, planStatus, isOnHoliday } from "@/lib/plan";
...
export async function sendDailyBriefing(profile, supabase) {
  ...
  // Guard #1 — if the plan is complete or never set up, don't send today's
  // workout email. Silently skip; the user isn't expecting one.
  if (status !== "active") {
    return { skipped: `plan status: ${status}` };
  }
  ...
  // Guard #2 — user is on holiday today. Skip email entirely so we don't
  // pester them about workouts they've explicitly paused.
  const { data: pauses } = await supabase
    .from("plan_pauses")
    .select("starts_on, ends_on, reason")
    .eq("user_id", profile.id);
  if (isOnHoliday(today, pauses || [])) {
    return { skipped: "on holiday" };
  }
